...
- **Exception — trivial cost-line price updates.** When a receipt only moves a `cost.groups[].items[]` entry from `note: 估算` to a confirmed amount (and at most updates the corresponding day item's `tickets[].tag/value` to match), commit directly to `master` — the user has already approved by providing the receipt. This covers receipt → trip updates whose diff is essentially a single `amount:` + label/tag swap. Anything that touches logistics (hotel/flight substitution, itinerary reordering, new or removed items) still goes through the normal PR-and-approval flow.
